...
Matrix calculate_sequence(Matrix *matrices, char *m_sequence, char *operators, 
                          int m_count, int seq_len);
...
Matrix check_for_multiplication(
    Matrix *matrices, char *m_sequence, char *operators,
...
Matrix find_by_name(char name, Matrix *matrices, int m_count);

/*Loads sequence of Matrices from stdin and returns their count*/
int input_matrices(Matrix *matrices);
...
Matrix create_matrix(int rows, int cols, int *values, char name);
...
int main(int argc, char *argv[])
{
  ...
  Matrix result = calculate_sequence(matrices, m_sequence, operators, m_count, 
                                    seq_len);
...
/*Calculates the resulting matrix of a sequence and returns it*/
Matrix calculate_sequence(Matrix *matrices, char *m_sequence,
                                 char *operators, int m_count, int seq_len)
{
  //declare matrices m1, m2 to hold partial results
  Matrix m1 = copy_matrix(find_by_name(m_sequence[0], matrices, m_count));
  Matrix m2;
...
/*Checks if the next operator is '*' and calculates the multiplication
if it is, it processes the multiplication sets the operator to '#' */
Matrix check_for_multiplication(Matrix *matrices, char *m_sequence, 
                                char *operators, int current_index, int m_count)
{
  ...
  if (operators[current_index + 1] == '*')
  {
    ...
    Matrix m2 = check_for_multiplication(matrices, m_sequence, operators,
