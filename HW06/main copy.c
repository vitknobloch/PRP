...
struct Matrix calculate_sequence(
  struct Matrix *matrices, char operators[], int m_count);
...
int check_for_multiplication(
  struct Matrix *matrices, char operators[], int current_index);

/*Loads sequence of Matrices and operators from stdin and returns their count*/
int input_matrices(struct Matrix *matrices, char operators[]);
...
struct Matrix create_matrix(int rows, int cols, int *values);
...
int main(int argc, char *argv[])
{
  ...
  struct Matrix result = calculate_sequence(matrices, operators, m_count);
...
struct Matrix calculate_sequence(
  ...
  while(i < m_count){
    struct Matrix m1 = matrices[i]; //keeps the first matrix of the next step
    ...
    struct Matrix m2 = matrices[i]; //keeps the second matrix for the next step
...
int check_for_multiplication(
  ...
  if(operators[current_index + 1]  == '*'){
    ...
    struct Matrix m1 = matrices[current_index + 1];
    ...
    struct Matrix m2 = matrices[next_index];
