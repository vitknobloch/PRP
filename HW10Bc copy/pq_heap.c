...
{
   pq_heap_s *pq = allocate_memory(sizeof(pq_heap_s));
   pq->size = size;
   pq->len = 0;
   pq->cost = allocate_memory(sizeof(int) * size);
   pq->label = allocate_memory(sizeof(int) * size);
   pq->heapIDX = allocate_memory(sizeof(int) * size);
...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (
	 ...
	 && pq->heapIDX[label] != -1 //vertex with the label is in the pq 
      ...
      pq->cost[current_index] = cost; // update the cost, but heap property is not satified
      // assert(pq_is_heap(pq, 0));

      int parent_index = GET_PARENT(current_index);
      while(current_index > 0 && pq->cost[parent_index] > cost){
         pq_swap(_pq, parent_index, current_index);
